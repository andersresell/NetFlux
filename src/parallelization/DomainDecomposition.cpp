...
namespace geometry
{
                                           ...
                                           map<FaceElement, GhostDataPartition> const &internal_boundary_faces_glob)
    {
        ...
        Index num_interior_faces_loc = faces_loc.size() - FV_Grid::find_N_GHOST_cells(faces_loc);
    ...
    //             const Elements &vol_elements_glob = primal_grid_glob->get_vol_elements();
    //             const Vector<Vec3> &nodes_glob = primal_grid_glob->get_nodes();
    //             assert(volume_element_partition.size() == vol_elements_glob.size());
    ...
    //                     the node indices from global to local values.
    //                     --------------------------------------------------------------------*/
    //                     vol_elements_loc.add_element_local(e_type, element, glob_to_loc_node_id);
    //                 }
    //             }
    ...
    //             primal_grid = make_unique<PrimalGrid>(nodes_loc, vol_elements_loc, element_patches_loc);
    //         }
    //     }
    // }
}
namespace NF_METIS
{
    ...
    void GridCreator::reorder_faces_enitities(Index num_interior_faces, const Vector<Patch> &patches, Faces &faces, Elements &face_elements)
    {
